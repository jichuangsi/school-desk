...
public class TopActivityUtils {
    ...
    private static String TAG = "EL service TopActivityUtils";
    ...
    public static String getActivityByPackageName(Context context, String packageName) {
        ...
        try {
            ...
            list = new ArrayList<ResolveInfo>(1);
        }
        ...
        if(TextUtils.isEmpty(activityName)) {
            try {
                ...
                //e.printStackTrace();
            	//Log.e(TAG, "getActivityByPackageName fail:" + packageName) ;
	...
	public static boolean isStatAccessPermissionSet(Context context) {
    	...
    	if(Build.VERSION.SDK_INT>=21) {
            try {
                ...
                ret = (mode == AppOpsManager.MODE_ALLOWED);
                Log.d(TAG, "get APP_OPS_SERVICE mode=" + mode) ;
                return ret ;
            } catch(Exception e) {
                Log.d(TAG, "get APP_OPS_SERVICE fail") ;
            }
    ...
    private static String getTopPackageOnAndroidLPlus(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 22) {
            ...
            //在小于Android5.1时，原则上来讲，Google是允许使用getRunningAppProcesses()返回正在执行进程列表的，
        	//但是不排除某些手机改了这个api（如miui7某些版本，用的是Android5.0.2，很有可能用了Android6的部分代码），
        	//这个时候，再去试试UsageStats方案，能返回就用，返回null就算了
