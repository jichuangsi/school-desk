...
def do_build(cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode):	
    ...
    platform = sys.platform
    if platform == 'win32':
        ndk_module_path = 'NDK_MODULE_PATH=e:/cocosqt;%s;%s/external;%s/cocos' % (cocos_root, cocos_root, cocos_root)
    else:
    ...
    if ndk_build_param == None:
        command = '%s V=1 -j%d -C %s %s' % (ndk_path, num_of_cpu, app_android_root, ndk_module_path)
    else:
        command = '%s V=1 -j%d -C %s %s %s' % (ndk_path, num_of_cpu, app_android_root, ''.join(str(e) for e in ndk_build_param), ndk_module_path)
...
def build(ndk_build_param,android_platform,build_mode):
    ...
    cocos_root = "e:/cocosqt/c2d"
    
    #cocos_root = os.path.join(current_dir, "../../c2d")
    print "cocos_root="  
    print cocos_root
